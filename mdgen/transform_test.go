...
// newHookFailure builds a hookFailure with the most common fields set; lifted
// here from the original generate_test.go since the transform tests need it
// and we are not porting the rest of generate_test.go in this PR.
...
func TestToSpecForTableDrivenSpec(t *testing.T) {
	want := &spec{
		...
		BeforeSpecHookFailures: nil,
		AfterSpecHookFailures:  nil,
		Errors:                 make([]buildError, 0),
...
var specNameGenerationTests = []*specNameGenerationTest{
	{filepath.Join("Users", "gauge", "foo", "simple_specification.spec"), filepath.Join("Users", "gauge", "foo"), "simple_specification.md"},
	{filepath.Join("Users", "gauge", "foo", "simple_specification.spec"), filepath.Join("Users", "gauge"), filepath.ToSlash(filepath.Join("foo", "simple_specification.md"))},
	{"simple_specification.spec", "", "simple_specification.md"},
	{filepath.Join("Users", "gauge", "foo", "abcd1234.spec"), filepath.Join("Users", "gauge", "foo"), "abcd1234.md"},
	{filepath.Join("Users", "gauge", "foo", "bar", "simple_specification.spec"), filepath.Join("Users", "gauge", "foo"), filepath.ToSlash(filepath.Join("bar", "simple_specification.md"))},
	{filepath.Join("Users", "gauge", "foo", "bar", "simple_specification.spec"), "Users", filepath.ToSlash(filepath.Join("gauge", "foo", "bar", "simple_specification.md"))},
	{filepath.Join("Users", "gauge12", "fo_o", "b###$ar", "simple_specification.spec"), "Users", filepath.ToSlash(filepath.Join("gauge12", "fo_o", "b###$ar", "simple_specification.md"))},
}

func TestToMDFileName(t *testing.T) {
	for _, test := range specNameGenerationTests {
		got := toMDFileName(test.specName, test.projectRoot)
		want := test.MDFilename
		if got != want {
			t.Errorf("want:\n%q\ngot:\n%q\n", want, got)
		}
	}
...
var tableDrivenStatusComputeTests = []*tableDrivenStatusComputeTest{
	...
	{"all skip",
		...
		skip},
	{"spec before hook fail",
		&spec{BeforeSpecHookFailures: []*hookFailure{
			{HookName: "Some failure", TableRowIndex: 0},
		}, Datatable: &table{Headers: []string{"foo"}, Rows: []*row{{Cells: []string{"foo1"}}}},
			Scenarios: []*scenario{
				{ExecutionStatus: pass, TableRowIndex: 0},
			}},
		fail},
	{"spec after hook fail",
		&spec{AfterSpecHookFailures: []*hookFailure{
			{HookName: "Some failure", TableRowIndex: 0},
		}, Datatable: &table{Headers: []string{"foo"}, Rows: []*row{{Cells: []string{"foo1"}}}},
			Scenarios: []*scenario{
				{ExecutionStatus: pass, TableRowIndex: 0},
